...
#define LARGE_SIZE (MTGPDC_N + THREAD_NUM)
#define PARAM_NUM_MAX mtgpdc_params_11213_num
#define BLOCK_NUM_MAX 200 /* max about 5000, if parameter sets exist */
#define TBL_SIZE 16
...
__device__ void status_read(uint32_t status[2][LARGE_SIZE],
    ...
    status[0][tid] = x >> 32;
    status[1][tid] = x & 0xffffffff;
    if (tid < N - THREAD_NUM) {
	x = d_status[bid].status[THREAD_NUM + tid];
	status[0][THREAD_NUM + tid] = x >> 32;
	status[1][THREAD_NUM + tid] = x & 0xffffffff;
    }
    __syncthreads();
}
...
__device__ void status_write(mtgp64_kernel_status_t *d_status,
    ...
    x = (uint64_t)status[0][(idx + tid) % LARGE_SIZE] << 32;
    x = x | status[1][(idx + tid) % LARGE_SIZE];
    d_status[bid].status[tid] = x;
    if (tid < N - THREAD_NUM) {
	x = (uint64_t)status[0][(THREAD_NUM + idx + tid) % LARGE_SIZE] << 32;
	x = x | status[1][(THREAD_NUM + idx + tid) % LARGE_SIZE];
...
__global__ void mtgp64_uint64_kernel(mtgp64_kernel_status_t* d_status,
	...
	para_rec(&YH,
		 &YL,
		 status[0][j % LARGE_SIZE],
		 status[1][j % LARGE_SIZE],
		 status[0][(j + 1) % LARGE_SIZE],
		 status[1][(j + 1) % LARGE_SIZE],
		 status[0][(j + pos) % LARGE_SIZE],
		 status[1][(j + pos) % LARGE_SIZE],
		 bid);
	status[0][(j + MTGPDC_N) % LARGE_SIZE] = YH;
	status[1][(j + MTGPDC_N) % LARGE_SIZE] = YL;
	o = temper(YH,
		   YL,
		   status[1][(j + pos - 1) % LARGE_SIZE],
		   bid);
	d_data[size * bid + i + tid] = o;
	j = (j + THREAD_NUM) % LARGE_SIZE;
	__syncthreads();
    }
    j = (j + 2 * LARGE_SIZE - THREAD_NUM - tid) % LARGE_SIZE;
    // write back status for next call
    status_write(d_status, status, bid, tid, j);
}
