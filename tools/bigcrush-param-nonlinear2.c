...
void fill_table32_nonlinear(uint32_t dest[], uint32_t source[], int size);
void fill_table32(uint32_t dest[], uint32_t source[], int size);
void fill_table64_nonlinear(uint64_t dest[], uint64_t source[], int size);
...
void read_params32(mtgp32_params_fast_t *params, char *argv[]) {
    ...
    fill_table32_nonlinear(params->tmp_tbl, tmp, 16);
    for (int i = 0; i < 16; i++) {
...
void fill_table32_nonlinear(uint32_t dest[], uint32_t source[], int size) {
    const uint32_t sbox[16] = {0, 12, 6, 9, 3, 15, 2, 10, 8,
			 11, 14, 1, 4, 7, 5, 13};
    ...
    for (int i = 0; i < size; i++) {
	dest[i] = (dest[i] & 0xfffffff0) | sbox[i];
    }
}
