...
static void print_minpoly(vec_GF2& vec, GF2X& poly);
static void print_jump_poly(GF2X& poly);
...
int main(int argc, char *argv[]) {
    ...
    jump_step = strtol(argv[4], NULL, 10);
    if (errno) {
...
static void print_minpoly(vec_GF2& vec, GF2X& minpoly) {
...
static void print_jump_poly(GF2X& minpoly)
{
    ...
    printf("jump_poly for %ld steps is\n", jump_step);
...
static void u32_print_poly(int mexp, int no, int seed) {
    ...
    print_minpoly(vec, poly);
    print_jump_poly(poly);
    mtgp32_free(&mtgp32);
}
