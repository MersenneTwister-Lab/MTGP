...
static void test(mtgp32_fast_t * sfmt, GF2X& poly);
...
static void speed(mtgp32_fast_t * sfmt, GF2X& characteristic);
...
int main(int argc, char * argv[]) {
    ...
    params += no;
    rc = mtgp32_init(&mtgp, params, seed);
...
{
    printf("idx = %d                             idx = %d\n",
	   a->status->idx, b->status->idx);
    for (int i = 0; (i < 10) && (i < a->status->large_size); i++) {
	printf("%08"PRIx32, a->status->array[i]);
	printf(" ");
	printf("%08"PRIx32, b->status->array[i]);
	printf("\n");
    }
}
