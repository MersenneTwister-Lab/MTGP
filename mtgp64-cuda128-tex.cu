 ...
 * did not work.
 */
__constant__ uint32_t high_mask = 0xffffffff;
__constant__ uint32_t low_mask =  0xff800000;

/**
 * Shared memory
 * The generator's internal status vector.
 */
__shared__ uint32_t status[2][LARGE_SIZE]; /* 128 * 4 elements, 4096 bytes. */
...
__device__ void status_read(uint32_t status[2][LARGE_SIZE],
    ...
    status[0][5 * THREAD_NUM - N + tid] = x >> 32;
    status[1][5 * THREAD_NUM - N + THREAD_NUM + tid] = x & 0xffffffff;
    if (tid < N - THREAD_NUM) {
	x = d_status[bid].status[2 * THREAD_NUM + tid];
	status[0][6 * THREAD_NUM - N + tid] = x >> 32;
	status[1][6 * THREAD_NUM - N + tid] = x & 0xffffffff;
    }
    __syncthreads();
}
...
__device__ void status_write(mtgp64_kernel_status_t *d_status,
    ...
    x = (uint64_t)status[0][5 * THREAD_NUM - N + tid] << 32;
    x = x | status[1][5 * THREAD_NUM - N + tid];
    d_status[bid].status[THREAD_NUM + tid] = x;
    if (tid < N - THREAD_NUM) {
	x = (uint64_t)status[0][6 * THREAD_NUM - N + tid] << 32;
	x = x | status[1][6 * THREAD_NUM - N + tid];
	d_status[bid].status[2 * THREAD_NUM + tid] = x;
    }
    __syncthreads();
}
...
__global__ void mtgp64_uint64_kernel(mtgp64_kernel_status_t* d_status,
	...
	para_rec(&YH,
		 &YL,
		 status[0][(5 * THREAD_NUM - N + tid) % LARGE_SIZE],
		 status[1][(5 * THREAD_NUM - N + tid) % LARGE_SIZE],
		 status[0][(5 * THREAD_NUM - N + tid + 1) % LARGE_SIZE],
		 status[1][(5 * THREAD_NUM - N + tid + 1) % LARGE_SIZE],
		 status[0][(5 * THREAD_NUM - N + tid + pos) % LARGE_SIZE],
		 status[1][(5 * THREAD_NUM - N + tid + pos) % LARGE_SIZE],
		 bid);
	status[0][tid + THREAD_NUM] = YH;
	status[1][tid + THREAD_NUM] = YL;
	o = temper(YH,
		   YL,
		   status[1][(5 * THREAD_NUM - N + tid + pos - 1) % LARGE_SIZE],
		   bid);
	...
	para_rec(&YH,
		 &YL,
		 status[0][6 * THREAD_NUM - N + tid],
		 status[1][6 * THREAD_NUM - N + tid],
		 status[0][6 * THREAD_NUM - N + tid + 1],
		 status[1][6 * THREAD_NUM - N + tid + 1],
		 status[0][6 * THREAD_NUM - N + tid + pos],
		 status[1][6 * THREAD_NUM - N + tid + pos],
		 bid);
	status[0][tid + 2 * THREAD_NUM] = YH;
	status[1][tid + 2 * THREAD_NUM] = YL;
	o = temper(YH,
		   YL,
		   status[1][6 * THREAD_NUM - N + tid + pos - 1],
		   bid);
	...
	para_rec(&YH,
		 &YL,
		 status[0][3 * THREAD_NUM - N + tid],
		 status[1][3 * THREAD_NUM - N + tid],
		 status[0][3 * THREAD_NUM - N + tid + 1],
		 status[1][3 * THREAD_NUM - N + tid + 1],
		 status[0][3 * THREAD_NUM - N + tid + pos],
		 status[1][3 * THREAD_NUM - N + tid + pos],
		 bid);
	status[0][tid + 3 * THREAD_NUM] = YH;
	status[1][tid + 3 * THREAD_NUM] = YL;
	o = temper(YH,
		   YL,
		   status[1][3 * THREAD_NUM - N + tid + pos - 1],
		   bid);
...
int main(int argc, char** argv)
{
    ...
    make_constant(mtgp64_params_fast_23209, block_num);
    make_texture(mtgp64_params_fast_23209, d_texture, block_num);
    make_kernel_data(d_status, mtgp64_params_fast_23209, block_num);
